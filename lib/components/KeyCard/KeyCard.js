...
function getErrors(localItemInfo, keys) {
  var errors = localItemInfo.get('errors', new _immutable.Map());
  ...
  if (!_immutable.List.isList(keys)) {
    keys = new _immutable.List([keys]);
  }

  return errors.filter(function (value, errorField) {
    return keys.includes(errorField);
  });
}
/**
 * Keycard
...
function (_React$Component) {
  ...
  _createClass(KeyCard, null, [{
    ...
    value: function renderedErrorInputMessage(errorKey, localItemInfo) {
      var errors = getErrors(localItemInfo, errorKey);

      if (errors) {
        return errors.valueSeq().map(function (suberrors) {
          if (_immutable.Map.isMap(suberrors) || _immutable.List.isList(suberrors)) {
            return suberrors.valueSeq().map(function (error) {
  ...
  _createClass(KeyCard, [{
    ...
    value: function renderedInputOneKeyCard(type) {
      var errorKey = new _immutable.List(['cardNumber', 'data.cardNumber']);
      var currentId = this.props.localItemInfo.get('skierIndex');
